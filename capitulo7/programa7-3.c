...
void main(void)
{
    ...
    caracter de terminacion '\n' a la posicion de memoria a la que apunta la variable
     cad0 -apuntador del tipo cadena de caracteres. */

     char cad1[20] = "hola";        /* Se asignan cuatro caracteres mas el caracter
     de terminacion a la variable tipo char cad1. Observa que cad1 tiene espacio
     para 20 caracteres.*/
...
printf("\nLa cadena cad1 es: ");
printf("%s", cad1);
/* La funcion printd, con el formato de variable %s, tambien se puede utilizar
...
ello esta funcion unicamente es util si conocemos con anticipacion
que la cadena que vamos a leer esta formada por una sola palabra. */
printf("%s", cad5);
fflush(stdin);

char p;
int i = 0;
/* La declaracion de variables siempre se debe realizar en la parte inicial
del programa. En este caso se colocan en esta seccion (char p e int i = 0) para
...
while ((p = getchar())!= '\n')
    cad6[i++] = p;
   cad6[i] = '\0';
   /*Al final de la caena se incorpora el caracter de terminacion NULL para
