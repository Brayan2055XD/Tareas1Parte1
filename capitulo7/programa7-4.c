...
/* Declaracion de cadenas e caracteres y asignacion de valores. */
...
void main(void)
{
    ...
    char *cad1;
    gets(*cad1);    gets(cad1);
    /* Incorrecto. Ambas lecturas generan un erro en la ejecucion del programa.
    ...
    char cad1[];
    /*Incorrecto. Se genera un error en la compilacion del rograma, por que no
    se reserva el espacio correspondiente. */

    char cad2[20] = "Mexico";  /* Correcto. */
    puts(cad2);
    gets(cad2);
    /* El valor de una cadena (declarada como cadena[longitud]) se puede
    modificar por medio de lecturas o utilizando funciones de la biblioteca string.h
    (ejemplo 7.6). */
    puts(cad2);
    cad2[10] = "Guatemala";
    /* Incorrecto. Observa cuidadosamente el caso anterior y analiza la diferencia
    que existe con este. Aqui se produce un error en la compilation del programa,
